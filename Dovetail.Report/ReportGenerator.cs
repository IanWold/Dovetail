...
internal static class ReportGenerator
{
    internal static async Task<int> RunAsync(string[] args, TextWriter stdout, TextWriter stderr)
    {
        ...
        for (var i = 0; i < args.Length; i++)
        {
            switch (args[i])
            {
                ...
                default:
                    stderr.WriteLine($"Unrecognized argument: {args[i]}");
                    return 1;
            }
        }
        ...
        {
            stderr.WriteLine("Specify only one of --project or --solution.");
            return 1;
        }
        ...
        if (projectPath is null && solutionPath is null)
        {
            ...
            {
                stderr.WriteLine($"Multiple project files found in {cwd}; specify --project.");
                return 1;
            }
            else
            {
                ...
                {
                    stderr.WriteLine($"Multiple solution files found in {cwd}; specify --solution.");
                    return 1;
                }
                else
                {
                    stderr.WriteLine($"No project or solution file found in {cwd}; specify --project or --solution.");
                    return 1;
                }
            }
        }
        ...
        {
            stderr.WriteLine($"Solution file not found: {solutionPath}");
            return 1;
        }

        if (projectPath is not null && !File.Exists(projectPath))
        {
            stderr.WriteLine($"Project file not found: {projectPath}");
            return 1;
        }
        ...
        workspace.RegisterWorkspaceFailedHandler(e => stderr.WriteLine($"warning: {e.Diagnostic.Message}"));
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    stderr.WriteLine($"Could not compile project: {projectPath}");
                    return 1;
                }
        ...
        {
            stderr.WriteLine($"Failed to load {solutionPath ?? projectPath}: {ex.Message}");
            return 1;
        }
        ...
        foreach (var compilation in compilations)
        {
            ...
            if (errors.Length > 0)
            {
                stderr.WriteLine($"'{compilation.AssemblyName}' has {errors.Length} compile error(s); fix these before generating a report:");
        ...
        stdout.WriteLine($"Wrote report for {sortedGraphs.Length} pipeline(s) to {outputPath}");

        return 0;
    }

    private static Version? GetReferencedDovetailVersion(Compilation compilation) =>
    ...
    private static void WriteEmbeddedAsset(string logicalName, string destinationPath)
    {
